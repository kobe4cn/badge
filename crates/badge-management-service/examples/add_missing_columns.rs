...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    sqlx::query(
        "ALTER TABLE redemption_orders ADD COLUMN IF NOT EXISTS idempotency_key VARCHAR(200)",
    )
    .execute(&pool)
    .await?;
    ...
    sqlx::query(
        "ALTER TABLE badge_redemption_rules ADD COLUMN IF NOT EXISTS enabled BOOLEAN DEFAULT true",
    )
    .execute(&pool)
    .await?;
