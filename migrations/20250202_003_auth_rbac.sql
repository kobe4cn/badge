...
-- ============================================
-- 创建默认管理员用户
-- 密码: admin123 (Argon2id 加密)
-- 生成命令: echo -n "admin123" | argon2 $(openssl rand -base64 16) -id -e
-- ============================================
INSERT INTO admin_user (username, password_hash, display_name, status)
VALUES (
    'admin',
    -- 使用 bcrypt 格式作为临时方案，后续可换成 argon2id
    '$2a$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/X4.F5g5Y5cPfJqzXe', -- admin123
    '系统管理员',
    'ACTIVE'
)
...
VALUES (
    'viewer',
    '$2a$12$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', -- viewer123
    '只读用户',
    'ACTIVE'
)
ON CONFLICT (username) DO NOTHING;
